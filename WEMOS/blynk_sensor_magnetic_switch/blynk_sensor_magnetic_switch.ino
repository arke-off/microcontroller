...
void estadosensor(){
int estado;
int estadoAnterior = LOW;  // Estado previo del sensor
unsigned long tiempoDeEspera = 1000;  // Tiempo de espera en milisegundos (ajusta según sea necesario)
unsigned long ultimoCambio = 0;  // Momento del último cambio de estado

estado = digitalRead(22);

  if (estado != estadoAnterior && millis() - ultimoCambio >= tiempoDeEspera) {  // Verifica si el estado ha cambiado y ha pasado el tiempo de espera

    ultimoCambio = millis();  // Actualiza el último cambio de estado
